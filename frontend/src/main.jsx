...
function InventoryLikePageShell({tab,setTab,menu=[],me,logout,children}){
  ...
  const roleName = me?.username || (me?.role==="platform_admin" ? "admin@smartinv.com" : "store@smartinv.com");
  const nav = Array.isArray(menu) && menu.length ? menu : APP_USER_PAGES;
...
function Operations({me,setTab,logout,hideChrome=false}){
  ...
  function backupProject(){
    downloadJSON(`smartinventory_backup_${new Date().toISOString().slice(0,10)}.json`,{products,associations,detectedEpcs,backup_date:new Date().toISOString()});
  }
...
function LocalData(){
  ...
  function saveProject(){
    ...
    downloadJSON(`smartinventory_backup_${new Date().toISOString().slice(0,10)}.json`, backup);
  ...
  function exportFullAudit(){
    ...
    exportCSV("audit_complet_smartinventory.csv",rows,cols);
  }
