...
const LS_PRODUCTS="smart_inventory_products_v7";
const LS_ASSOC="smart_inventory_associations_v7";
const LS_DETECTED_EPCS="smart_inventory_detected_identifiers_v7";
const LEGACY_LS_PRODUCTS=atob("cmZpZF92N19wcm9kdWN0cw==");
const LEGACY_LS_ASSOC=atob("cmZpZF92N19hc3NvY2lhdGlvbnM=");
const LEGACY_LS_DETECTED_EPCS=atob("cmZpZF92N19kZXRlY3RlZF9lcGNz");
...
const APP_ADMIN_PAGES = [
  ...
  {id:"platform", label:"Clients", icon:"platform"},
...
function loadLS(k,def,legacyKey=""){ try{
  const raw = localStorage.getItem(k) ?? (legacyKey ? localStorage.getItem(legacyKey) : null) ?? "";
  return JSON.parse(raw||"");
}catch{return def} }
function norm(v){ return String(v ?? "").trim().replaceAll(" ","").replaceAll("-","").toUpperCase(); }
function assocIdentifierRaw(a){ return a?.Identifiant || a?.EPC || a?.Tag || a?.tag || ""; }
function assocIdentifier(a){ return norm(assocIdentifierRaw(a)); }
function isLikelyEpc(v){
  const s=norm(v);
  if(!s || ["EPC","TAG","DATE","PID","PRODUIT","PRODUCT"].includes(s)) return false;
...
function useLocalStore(){
  const [productsState,setProductsState]=useState(()=>loadLS(LS_PRODUCTS,[],LEGACY_LS_PRODUCTS));
  const [associationsState,setAssociationsState]=useState(()=>loadLS(LS_ASSOC,[],LEGACY_LS_ASSOC));
  const [detectedEpcsState,setDetectedEpcsState]=useState(()=>loadLS(LS_DETECTED_EPCS,[],LEGACY_LS_DETECTED_EPCS));
...
function SmartInventoryLogo({className=""}){
  return <div className={`smartSidebarLogo ${className}`.trim()} aria-label="Inventory Connect">
    <SidebarBrandIcon className="smartSidebarLogoIcon"/>
    <div className="smartSidebarLogoText">
      <span className="smartSidebarLogoSmart">Inventory</span>
      <span className="smartSidebarLogoInventory">Connect</span>
    </div>
  </div>
}
...
function DashIcon({name}){
  ...
  if(name==="signal") return <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M5 13.5a10 10 0 0 1 14 0" {...common}/><path d="M8.2 16.5a5.8 5.8 0 0 1 7.6 0" {...common}/><path d="M11.3 19.3a1.1 1.1 0 0 1 1.4 0" {...common}/></svg>;
...
function App(){
  ...
  const pageTitle =
    ...
    tab==="dashboardAdmin" ? "Publicités" : "Inventory Connect";
    ...
    <section className="whiteMain">
      <header className="whiteTopbar">
        ...
        <div className="shuffleAppChromeLabel" aria-hidden="true">
          ...
          <code>app.inventoryconnect.app/{tab}</code>
          <em>2026 Stable</em>
        </div>
      ...
      <footer className="whiteFooter">© 2026 Inventory Connect. All rights reserved.</footer>
    </section>
  </div>
}
  ...
  return <div className="shuffleUnifiedShell">
    <header className="shuffleUnifiedNav">
      <div className="shuffleUnifiedNavInner">
        <button type="button" className="shuffleUnifiedBrand" onClick={()=>setTab("dashboard")} aria-label="Inventory Connect Dashboard">
          <span className="shuffleUnifiedLogo"><SidebarBrandIcon/></span>
          <b>Inventory Connect</b>
        </button>
...
function InventoryLikePageShell({tab,setTab,menu=[],me,logout,children}){
  ...
  const accountName = me?.pharmacy_name || "Inventory Connect";
  const roleName = me?.username || (me?.role==="platform_admin" ? "admin@inventoryconnect.app" : "store@inventoryconnect.app");
  const nav = Array.isArray(menu) && menu.length ? menu : APP_USER_PAGES;
  ...
  return <div className="dashRefShell dashRefShellV2 dashRefTopOnlyShell">
    <main className="dashRefMain dashRefMainTopOnly">
      <header className="dashRefGlobalHeader dashRefTopOnlyHeader">
        <button type="button" className="dashRefHeaderBrand" onClick={()=>setTab("dashboard")} aria-label="Inventory Connect Dashboard">
      ...
      <footer className="dashRefFooter">
        <div><span className="dashRefFooterLogo"><InvIcon name="cube"/></span><b>Inventory Connect</b></div>
        <p>© 2026 Inventory Connect. Tous droits réservés.</p>
...
function Operations({me,setTab,logout,hideChrome=false}){
  ...
  function importAssociations(file){
    ...
    Papa.parse(file,{header:true,skipEmptyLines:true,complete:(res)=>{
      const rows=res.data.map((r)=>({
        ...
        "Code barre 2":r["Code barre 2"]||"",
        Identifiant:norm(r.Identifiant||r.identifiant||r.EPC||r.epc||r.Tag||r.tag||""),
        Date:r.Date||new Date().toISOString()
      })).filter(x=>x.Identifiant);
  ...
  function importDetectedEpc(file){
    ...
    Papa.parse(file,{header:false,skipEmptyLines:true,complete:(res)=>{
      ...
      downloadJSON(`identifiants_detectes_${new Date().toISOString().slice(0,10)}.json`,{identifiants:rows,date:new Date().toISOString()});
  ...
  function backupProject(){
    downloadJSON(`inventoryconnect_backup_${new Date().toISOString().slice(0,10)}.json`,{products,associations,detectedEpcs,backup_date:new Date().toISOString()});
  }
...
function CashRegister({me}={}){
  ...
  return <section className="cashRegisterPage">
              ...
              <table className="cashTable cashCountTable">
                ...
                <tbody>
                  {billDenominations.map(d=>{
                    ...
                    return <tr key={d.cents}>
                      <td>{d.label}</td>
                      <td><span className="qtyDisplay" aria-label={`Quantité affichée ${d.label}`}>{qty}</span></td>
    ...
    <div className="pageFooterLikeDashboard">© 2026 Inventory Connect. Tous droits réservés.</div>
  </section>
}
...
function CashDashboardAdmin(){
  ...
  return <section className="cashShuffleDashboard">
    ...
    <div className="cashShuffleGrid cashShuffleContentV142 cashShuffleContentV210 cashShuffleContentV211 cashShuffleContentV216 cashShuffleContentV217 cashShuffleContentV218">
      <div className="cashShuffleSection cashShuffleSectionV210">
        ...
        <CashShuffleCard title="Tot. dépenses" meta={limitMeta("Total mensuel", "monthlyExpenseLimitCents", monthlyExpenseLimitCents)} className="cashShuffleCardTall cashShuffleProgressCard cashShuffleExpensesLimitCard cashShuffleInsightCard cashShuffleV210Standard" dotTone="amber">
          <CashShuffleExpenseProgress value={expensesProgress} cents={monthlyExpensesCents} limitCents={monthlyExpenseLimitCents} />
        </CashShuffleCard>

        <CashShuffleCard title="Tot. montant manquant" meta={cardMeta("Total mensuel")} badge={<InvIcon name="cash" />} badgeClassName="cashShuffleBadgeIcon cashShuffleBadgeDanger" className="cashShuffleCardTall cashShuffleMonthlyAmountCard cashShuffleMonthlyShortageCard cashShuffleV210Standard">
          <CashShuffleMonthlySummary cents={monthlyShortageCents} tone="negative" emptyText="Aucun montant manquant enregistré" />
        </CashShuffleCard>

        <CashShuffleCard title="Tot. montant surplus" meta={cardMeta("Total mensuel")} badge={<InvIcon name="sync" />} badgeClassName="cashShuffleBadgeIcon cashShuffleBadgeSuccess" className="cashShuffleCardTall cashShuffleMonthlyAmountCard cashShuffleMonthlySurplusCard cashShuffleV210Standard cashShuffleV211KpiLikeShortage cashShuffleV218MatchModelDim">
          <CashShuffleMonthlySummary cents={monthlySurplusCents} tone="positive" emptyText="Aucun surplus enregistré" />
        </CashShuffleCard>

        <CashShuffleCard title="Balance due" meta={cardMeta("Jour sélectionné")} badge={<InvIcon name="lab" />} badgeClassName="cashShuffleBadgeIcon cashShuffleBadgeIndigo" className="cashShuffleMini cashShuffleBalanceDueActionCard cashShuffleV210Standard cashShuffleV211KpiLikeShortage cashShuffleV218MatchModelDim">
        ...
        <CashShuffleCard title="Montant manquant" meta={cardMeta("Jour sélectionné")} badge={<InvIcon name="cash" />} badgeClassName="cashShuffleBadgeIcon cashShuffleBadgeDanger" className="cashShuffleMini cashShuffleV210Standard cashShuffleV211KpiLikeShortage cashShuffleV218MatchModelDim">
          <CashShuffleAmount cents={selectedMetrics.shortageCents} tone="negative" />
        </CashShuffleCard>

        <CashShuffleCard title="Montant surplus" meta={cardMeta("Jour sélectionné")} badge={<InvIcon name="sync" />} badgeClassName="cashShuffleBadgeIcon cashShuffleBadgeSuccess" className="cashShuffleMini cashShuffleV210Standard cashShuffleV211KpiLikeShortage cashShuffleV218MatchModelDim">
          <CashShuffleAmount cents={selectedMetrics.surplusCents} tone="positive" />
        </CashShuffleCard>

        <CashShuffleCard title="Retiré" meta={cardMeta("Jour sélectionné")} badge={<InvIcon name="upload" />} badgeClassName="cashShuffleBadgeIcon cashShuffleBadgeBlue" className="cashShuffleMini cashShuffleV210Standard cashShuffleV211KpiLikeShortage cashShuffleV218MatchModelDim">
          <CashShuffleAmount cents={selectedMetrics.withdrawnCents} />
        </CashShuffleCard>

        <CashShuffleCard title="Nouvelle C. fermeture" meta={cardMeta("Jour sélectionné")} badge={<InvIcon name="cash" />} badgeClassName="cashShuffleBadgeIcon cashShuffleBadgeIndigo" className="cashShuffleWide cashShuffleV210Standard cashShuffleV211KpiLikeShortage cashShuffleV218MatchModelDim">
          <CashShuffleAmount cents={selectedMetrics.closingRealCents} xl />
        </CashShuffleCard>

        <CashShuffleCard title="Vente en espèce + règlement de crédit" meta={cardMeta("Jour sélectionné")} badge={<InvIcon name="cash" />} badgeClassName="cashShuffleBadgeIcon cashShuffleBadgeIndigo" className="cashShuffleWide cashShuffleV210Standard cashShuffleV211KpiLikeShortage cashShuffleV218MatchModelDim">
          <CashShuffleAmount cents={Number(selectedMetrics.salesCashCents || 0) + Number(selectedMetrics.creditSettlementCents || 0)} xl />
        </CashShuffleCard>

        <CashShuffleCard title="Dépenses" meta={cardMeta("Jour sélectionné")} badge={<InvIcon name="list" />} badgeClassName="cashShuffleBadgeIcon cashShuffleBadgeAmber" className="cashShuffleMini cashShuffleV210Standard cashShuffleV211KpiLikeShortage cashShuffleV218MatchModelDim">
          <CashShuffleAmount cents={selectedMetrics.expensesCents} />
        </CashShuffleCard>

        <CashShuffleCard title="Alertes anomalies" meta={cardMeta(anomalyMetaText)} className="cashShuffleCardTall cashShuffleAnomalyCard cashShuffleV210Tall" dotTone={anomalyAlerts.length ? "amber" : "emerald"}>
          <CashAnomalyList />
        </CashShuffleCard>

        <CashShuffleCard title="C. fermeture (théorique)" meta={cardMeta("Jour sélectionné")} badge={<InvIcon name="grid" />} badgeClassName="cashShuffleBadgeIcon cashShuffleBadgeIndigo" className="cashShuffleWide cashShuffleV210Standard cashShuffleV211KpiLikeShortage cashShuffleV218MatchModelDim">
          <CashShuffleAmount cents={selectedMetrics.closingCalculatedCents} xl />
        </CashShuffleCard>

        <CashShuffleCard title="C. fermeture (comptée)" meta={cardMeta("Jour sélectionné")} badge={<InvIcon name="lab" />} badgeClassName="cashShuffleBadgeIcon cashShuffleBadgeIndigo" className="cashShuffleWide cashShuffleWideDelta cashShuffleV210Standard cashShuffleV211KpiLikeShortage cashShuffleV218MatchModelDim">
...
function Login({setToken}){
  ...
  const LoginNav=()=> <nav className="shuffleExactLoginNav">
    <div className="shuffleExactLoginNavInner">
      <a className="shuffleExactLoginBrand" href="#home" onClick={goLanding} aria-label="Inventory Connect accueil">
        <span className="shuffleExactLoginBrandIcon"><SidebarBrandIcon/></span>
        <span>Inventory Connect</span>
      </a>
...
function AIAssistant(){
  ...
  function exportRowsToPDF(q){
    const intent=chooseRowsByIntent(q);
    const {filtered,applied}=filterRows(intent.base,q);
    const title=`Inventory Connect - Rapport ${intent.label}`;
    const lines=[
  ...
  function buildLocalResponse(q){
    ...
    if(/doublon|duplicate|meme identifiant|même identifiant/.test(text)) return duplicateReport();
...
function Association({setTab=()=>{}, me, logout=()=>{}}){
  ...
  const rows = associations.filter(a=>{
    const s = `${a.PID||""} ${a.Produit||""} ${assocIdentifierRaw(a)||""} ${a["Code barre 1"]||""} ${a["Code barre 2"]||""}`.toLowerCase();
  ...
  function exportRows(){ const out=rows.map(r=>({...r, Identifiant:assocIdentifierRaw(r)})); exportCSV("associations.csv", out, ["PID","Produit","Code barre 1","Code barre 2","Identifiant","Date"]); }
  const nav = [
    ...
    <nav className="shuffleAssocNav">
      <div className="shuffleAssocNavInner">
        <div className="shuffleAssocNavLeft">
          <button type="button" className="shuffleAssocBrand" onClick={()=>setTab("dashboard")} aria-label="Inventory Connect dashboard">
        ...
        <div className="shuffleAssocProfileWrap">
          ...
          <div className="shuffleAssocUserText">
            <p>{me?.pharmacy_name || "Store Démo"}</p>
        ...
        <div className="shuffleAssocTableWrap">
          <table className="shuffleAssocTable">
            ...
            <tbody>
              ...
              </tr> : rows.map((r,i)=><tr key={`${r.PID||""}-${assocIdentifierRaw(r)||""}-${i}`}>
                <td>{r.PID || "—"}</td>
                <td>{r.Produit || "—"}</td>
                <td>{r["Code barre 1"] || "—"}</td>
                <td>{r["Code barre 2"] || "—"}</td>
                <td><code>{assocIdentifierRaw(r) || "—"}</code></td>
                <td>{r.Date || "—"}</td>
    ...
    <footer className="shuffleAssocFooter">
      <div className="shuffleAssocFooterInner">
        <div className="shuffleAssocFooterBrand">
          ...
          <small>© 2026 Inventory Connect. Tous droits réservés.</small>
        </div>
  ...
  return <div className="shuffleInvPage withInternalTopNav">
    <header className="internalTopDashNav" aria-label="Navigation dashboard">
      <div className="internalTopDashNavInner">
        <button type="button" className="internalTopDashBrand" onClick={()=>setTab("dashboard")} aria-label="Inventory Connect dashboard">
        ...
        <div className="internalTopDashAccount">
          <div className="shuffleInvStoreBadge"><InvIcon name="store"/><span>{me?.pharmacy_name || "Store Démo"}</span></div>
    ...
    <footer className="shuffleInvFooter">
      <div className="shuffleInvFooterInner">
        <div className="shuffleInvFooterBrand"><span><InvIcon name="cube"/></span><b>Inventory Connect</b></div>
        <p>© 2026 Inventory Connect. Tous droits réservés.</p>
...
function LocalData(){
  ...
  function saveProject(){
    ...
    downloadJSON(`inventoryconnect_backup_${new Date().toISOString().slice(0,10)}.json`, backup);
  ...
  function exportDuplicateEpcReport(){
    const counts={};
    associations.forEach(a=>{const e=assocIdentifier(a); if(e) counts[e]=(counts[e]||0)+1;});
    const rows=associations.filter(a=>counts[assocIdentifier(a)]>1).map(a=>({...a,Identifiant:assocIdentifierRaw(a),"Anomalie":"Doublon identifiant"}));
    const cols=["PID","Produit","Code barre 1","Code barre 2","Identifiant","Date","Anomalie"];
    exportCSV("rapport_doublons_identifiants.csv",rows,cols);
  }

  function exportFullAudit(){
    const associatedPids=new Set(associations.map(a=>String(a.PID)));
    const rows=products.map(p=>{
      const linked=associations.filter(a=>String(a.PID)===String(p.PID)).map(a=>assocIdentifierRaw(a)).join(", ");
    ...
    exportCSV("audit_complet_inventory_connect.csv",rows,cols);
  }
...
return <section className="platformPage platformUsersPage">
      ...
      <div className="scanModal platformStoreModal platformCreateUserModal" onClick={e=>e.stopPropagation()} role="dialog" aria-label="Créer un utilisateur">
        ...
        <div className="platformCreateModalBody">
          <section className="platformCreateSection">
            <h3>Identifiants</h3>
            <div className="platformCreateGrid">
              <label><span>User *</span><input placeholder="ex: store" value={username} onChange={e=>setUsername(e.target.value)}/></label>
              ...
              <label><span>User name *</span><input placeholder="ex: store Tanger" value={pharmacy} onChange={e=>setPharmacy(e.target.value)}/></label>
    ...
    {infoDraft && <aside className="platformInfoDrawer" role="dialog" aria-label="Infos utilisateur">
      ...
      <div className="platformInfoForm">
        ...
        <label className="full"><span>Store / User name</span><input value={infoDraft.pharmacy_name} onChange={e=>setInfoDraft({...infoDraft,pharmacy_name:e.target.value})} /></label>
...
function Dashboard({setTab, me, menu=[], logout}){
  ...
  const epcCounts={};
  associations.forEach(a=>{ const e=assocIdentifier(a); if(e) epcCounts[e]=(epcCounts[e]||0)+1; });
  ...
  const kpis=[
    ...
    {label:"Produits tagués", value:productsWithRfid, sub:productsWithRfid ? "Associés avec tag" : "Aucun tag détecté", tag:"Tags", tone:"emerald", icon:"link", action:()=>setTab("association")},
  ...
  return <section className="dashRefDashboard">
    <div className="dashRefHeroBlock">
      <div>
        <div className="dashRefStatusLine">
          <span className="dark">app.inventoryconnect.app/dashboard</span>
          <span>Mode stable</span>
          <span className="success">Synchronisation active</span>
        </div>
        <h1>Dashboard store</h1>
        <h2>suivi des tags en temps réel.</h2>
        <p>Suivi en temps réel de la couverture de l'activité de votre store, avec une lecture claire des produits tagués, des transactions et des alertes à traiter.</p>
      </div>
    ...
    <div className="dashRefTwoColumns">
      <section className="dashRefCoverageCard">
        <div className="dashRefSectionHead"><div><b>Taux de couverture</b><span>Pourcentage de produits avec tag actif sur le total</span></div><em>À améliorer</em></div>
        <div className="dashRefCoverageBody">
          <div className="dashRefCircle"><span>{coverage}%</span></div>
          <div>
            <h3>{coverage>=80 ? "Votre store est bien équipé." : coverage>=50 ? "Votre couverture progresse." : "Votre couverture doit être améliorée."}</h3>
            <p>Vous avez étiqueté {coverage}% de vos produits dans votre store. Commencez ou continuez l'étiquetage pour améliorer votre suivi d'inventaire et fiabiliser les sorties de caisse.</p>
      ...
      <section className="dashRefAdCard">
        ...
        <h3>Espace partenaire store</h3>
        <div className="dashRefPartnerLine"><i><InvIcon name="sync"/></i><b>Prodecys</b></div>
        {activeDashboardAd?.image_url ? <img src={mediaUrl(activeDashboardAd.image_url)} alt="Publicité"/> : <p>Accédez à vos outils partenaires, gérez vos clients stores et suivez les performances de vos installations d’inventaire.</p>}
    ...
    <div className="dashRefBottomGrid">
      ...
      <section className="dashRefAlertsBlock">
        <div className="dashRefSectionHead"><div><b>Alertes prioritaires</b><span>Points à vérifier quand le store est active</span></div></div>
